...
def factor_modulus(n, d, e):
	...
	cf. Handbook of Applied Cryptography
...
class RSA:
	def __init__(self, p=None, q=None, n=None, d=None, e=DEFAULT_EXP):
		"""
		Init RSA instance using primes (p, q)
		or mod & priv exp (n, d)
		"""

		self.e = e
		if p and q:
			assert gmpy.is_prime(p), "p != prime"
			assert gmpy.is_prime(q), "q != prime"
			self.p = p
			self.q = q
		elif n and d:
			self.p, self.q = factor_modulus(n, d, e)
		else:
			raise ArgumentError("(p, q) || (n, d) must be supplied")
		self._calc_values()
	...
	def to_pem(self):
		"""
		ret OpenSSL-compatible PEM-encoded key
		"""
	...
	def to_der(self):
		"""
		ret params as OpenSSL-compatible DER-encoded key
		"""
