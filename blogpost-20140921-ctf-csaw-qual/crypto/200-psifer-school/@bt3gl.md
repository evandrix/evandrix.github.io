# CSAW CTF 2014 - Crypto 200: "psifer school"
...
def find_frequency(msg):
    ...
    list_freq.sort()
    return [freq_c / total_letters for (c, freq_c) in list_freq]
```
...
Together with the hint *length doesn't matter*, we get the following cipher:

```I'lcslraooh o rga tehhywvf.retFtelh mao ae af ostloh lusr bTsfnr, epawlltddaheoo aneviedr
ose rtyyng etn aini ft oooey hgbifecmoswuut!oa eeg ar rr h.u t. hylcg io we ph
ftooriysneirdriIa utyco gfl oostif sp u"+'""'+"flcnb roh tprn.o h```
...
If Python is not your thing, check out [scripts from other authors], in Pearl and in PHP.

**Hack all the things!**

[his cryptographic scheme]: http://en.wikipedia.org/wiki/Caesar_cipher
[exploit for this problem]: https://github.com/bt3gl/Gray-Hacker-and-PenTesting/blob/master/CTFs/2014-CSAW-CTF/cryptography/crypto-200/psifer-school.py
